...
/*
case1:
     select v1, min(v2) from t0 group by v1 having min(v2) < 2
                             |
                            \|/
     select prd_t0.v1, min(prd_t0.v2) from (select v1, v2 from t0 where v2 < 2) prd_t0 group by prd_t0.v1 having min(prd_t0.v2) < 2

case2:
     select v1, min(v2) from t0 group by v1 having max(v2) >= 2
                             |
                            \|/
     select prd_t0.v1, min(prd_t0.v2) from (select v1, v2 from t0 where v2 >= 2) prd_t0 group by prd_t0.v1 having max(prd_t0.v2) >= 2
 */
